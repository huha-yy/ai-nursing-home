...
回落 MAX(date)）。这张表**没有任何运行时派发机制写入**——数据全部来自
一次性灌入的演示批次：
...
2. 早期演示批次：2026-07-27 / 07-29 各 30 条（不在 seed 里，当时手工/脚本
   生成）；
3. ~~2026-08-25 调数（53%→83%）~~ 已被第 4 条覆盖为当日数据；
4. **2026-08-25 改版主批次**：`scripts/seed_work_orders_demo.py`——近 14 天
   （含当天）× 每天 24-34 单共 414 条，固定随机种子可重跑幂等（DELETE 后
   INSERT，表无唯一索引）；完成率口径 过去日 88-97% / 昨天 ~90% / **当天
   62%**（留足"待处理"演示）；staff_name 严格取老人所在楼栋的护理员，note
   按 24 种工单类型模板生成。07-27/07-29 旧批次仍在表里（30 天窗口内，
   /work-orders 侧栏可见），不在脚本管理范围。

**被问"完成率怎么来的"时的标准答案**：演示口径——按护理工单完成数/总数
（工单为演示样本，不随真实护理工作变化）。**不要**宣称它来自 ERP。

**与其他组件的关系**：大屏另外五个 ERP 组件均为真实口径可追述；完成率是
唯一例外，演示时避免把两者混为一谈。**/work-orders 页**（2026-08-25 改版为
日期侧栏 + 列表/详情 主从）数据源是新端点 `GET /api/nursing/work-orders`
（近 30 天全量、楼长会话按楼栋过滤），与完成率 KPI 同表同源，口径一致。
