...
**同日五补（演示数据完成率调至 83%）**：用户嫌完成率 53% 一片红。数据
真源：dato-postgres nursing_work_orders 最新日期 2026-07-29（30 单 16 完成），
dashboard 的 _eff_date 无当日数据时回落 MAX(date)。UPDATE 翻 9 张
（血压×3/翻身×2/鼻饲×2/防走失/口腔，备注本就是完成口吻）→ 25/30=83%
绿色档，留 5 张真实待办。种子文件不含 7-27/7-29 这批行（是历史演示
生成的），无需改 seed；make reset 全新库会走 7-21 全 TRUE 复制 = 100%，
同为绿档。

**同日六补（完成率环固定主题橙）**：用户拍板"一直都是橙色"——完成率
扇形+中心数字固定 #F39C12，删掉 60/80 分档变色的三元（原来 <60 整环
变红、≥80 变绿，和暖橙主题违和；且完成率白天爬坡，早上必红=狼来了）。
代价是颜色不再报警，数字本身仍可读；工单详情表里逐类型的红黄绿保留。

**同日七补（护理等级饼改暖色阶）**：绿/紫/蓝高饱和跳色与暖橙主题违和。
改为同族暖色阶且带语义：越深=照护越重（自理浅杏 #FAD7A1 → 半护琥珀
#F39C12 → 全护主橙 #E67E22 → 失智焦橙 #A04000 → 特护深棕 #6E2C00，
未定中性暖灰 #C9AE8C），前三档直接复用主题 token。调色板顺序即
_CARE_LEVEL_ORDER 定序，颜色从"随机分类色"升级为"强度编码"。

**同日八补（饼心标题精确居中）**：中心"36人 在院分级"原用画布系
top:'28%'，而饼心在 44%（画布固定 280px → 圆心 ≈123px），文字浮高
≈20px。改 top:100px（123 − 块高43/2）。title 定位是画布坐标不是饼心
坐标，居中要按"圆心 − 半块高"手算；改饼高时记得同步这行。

**同日九补（撤掉 KPI 卡内楼栋迷你条形）**：用户看了实际效果拍板"不能放
这里"——在院老人卡里的迷你柱状图拆除（HTML 容器 + renderKpiBuildings +
调用 + 4 条 CSS 全删）。各楼栋在院人数从此无前端展示；dashboard API 的
building_distribution 键保留未动（数据小，Python 改动要重建容器，不值当，
留作后续复用）。教训：卡面塞图表容易挤占 KPI 数字的主导地位，数字卡就
该纯粹是数字卡。

**同日十补（大屏数据溯源文档）**：用户点破原则"演示数据可以假，口径必须
真"。盘查结论：五个 ERP 组件口径均可追述，唯独完成率来自 Postgres 静态
演示表（无派发机制，83% 为 08-25 手调）。给了三个转正方案（分级护理
派单模型/照护覆盖率/只补文档），用户最终拍板**只补文档不动代码**。新增
DASHBOARD-DATA.md（docs/ 目录被 gitignore，故放仓库根）：逐组件数据来源口径表 + 完成率
静态演示声明（含 83% 调数全程）+ 分级护理派单模型的完整论证与实施清单
（材料已验证：ERP NursingLog 7 类护理类型、demo 脚本 cat_by_level 即
分级护理逻辑），供日后转正直接照做。

**同日十一补（KPI 图标/状态色全面暖色化）**：用户发来截图称"紫绿红图标
违和"。像素比对（417×384 裁切图，色块簇绿 #27AE60/紫 #8E44AD 精确命中
旧饼图色板）确认截图是 c760a1c 改版**前**的旧图——线上饼图早已是暖色阶，
用户浏览器没刷新。但排查 nursing.css 发现 live 上确有绿紫红残留，一并
暖色化（设计语言与饼图统一：同族暖色、深浅=轻重）：

- `--nh-accent` #27AE60 绿 → #B9770E 深金（kpi-ok 数值/trend-good/工单
  页进度条全部跟随变金）
- KPI 图标块：duty 绿→金 #D68910、assess 紫→棕 #8B5A2B、alert 红→焦橙
  #A04000（最紧急=最深），people 主橙/inventory 琥珀不变
- 三餐色带原"早橙/午绿/晚紫"→"早琥珀/午主橙/晚焦橙"（越晚越深），徽章同源
- 点餐动态 delivered 绿章→杏金 #FAD7A1/#7E5109
- 保留语义色：--nh-danger 红（告警/危险）、cancelled 粉章、菜品荤红素绿
  汤蓝（餐饮食物语义，此前已获用户认可）

排查方法论：色块簇 hex 分桶比对 git 历史色板，可断代截图新旧，不信视觉
模型叙述（本轮它又把深橙说成"无红色"以外的成分）。

**同日十二补（重点关注卡蓝色退场 + CSS 损坏块修复）**：用户点名"今日重点
关注老人"卡里的蓝色。定位：--nh-info #3498DB（"常规关注"分组头/圆点/老人
行左边条/行内圆点；院长视角 focus 有 warning+info 各 1 人所以可见，楼长
视角 focus 为空看不到）。暖化：token → #8D6E63 暖棕灰（常规=最轻最中性），
两处冷蓝底（分组头 #EEF3F9→#F4EFE9、告警页 info 徽章 #D6EAF8→#EFE6D9）
换暖纸底。至此重点卡三级=红/琥珀/暖棕，全暖。刻意保留：聊天页角色徽章
蓝（311）、汤品蓝点（1161，食物语义）。
...
**同日十四补（周报页：旁白与正文拆分）**：用户看 /reports 发现院长周报
正文里混着"📋 已读取技能说明/⚠️ 缺少 asyncpg/✅ 修复成功"等 agent 工具
调用旁白，问"思考部分是不是不该保存"。定位：OpenClaw 把旁白和最终报告
拼在同一串 payload 文本，_extract_step_summary 对报告类步骤原样整段返回。
修复：新增模块级 _split_report_text（正文=首个 markdown 标题行起；标题前
旁白+尾部"执行摘要"归 process 键），summary 是 /api/nursing/report 读取时
现算的——改函数即救回 8-14 历史记录，无需动库；raw 字段仍全量留档（审计
不受影响，旁白里"自动修复依赖"这类运维线索保得住）。前端：正文照常
simpleMd，旁白折叠进 <details>"⚙ 执行过程"；顺手补 simpleMd 的 H1 渲染
（"# 🏥…"之前不转标题）+ 该页绿状态点暖金化。测试 +3（116 passed）。

内容评估（答复用户）：四步数字内部可互相印证（336h/人均168h 算术对，
12328 成本合计对）；遗留两点——① 周期 08-14~08-20 但生成于 08-14（生成
时半周未发生，属预估口径），且距今 11 天未再触发；② flow 默认
building="3号楼"，"院长周报"实为单栋周报（报告内有诚实标注），全院化
需扩 flow，留待用户拍板。

**同日十五补（剥除周报尾部机器交接 JSON）**：用户点出财务科报告尾部的
"六、输出 JSON（供下一部门使用）"段没必要展示。_split_report_text 增加剥除
规则：① 有"输出 JSON"标题 → 标题起整段剥；② 无标题的裸 ```json 块 →
块后只剩 ≤120 字收尾话（总务科实际形态："以上结果可直接作为下一部门…
的输入。"）或为空则连收尾话一起剥，后面还有实质内容则当正文引用不动
（防误伤）。第一版用 50% 位置阈值+`$` 锚，短正文误挡/中部误咬两坑，改
"块后剩余长度"判据+finditer 从后往前。财务正文里"数据来源: 排班结果
（runId a3d5b40a）"是合理的溯源引用，保留。测试 +2（120 passed），
线上三报告验证：finance 2931→1878、logistics 2396→1437 字，无残留
json 块。raw 仍全量留档。

**同日十六补（/reports 一拉到底改主从结构——长列表页改造试点）**：用户
提出 /reports 是"一拉到底、没有结构没有分页没有嵌套跳转"的典型，要做
整体排版优化的试点。改造为**期次侧栏 + 报告详情**的主从结构：
- **API**：/api/nursing/report 加 offset/limit 参数（limit 钳 1~50），返回
  total + has_more；顺带砍掉没人渲染的 raw 字段（10 期 × 4 步 OpenClaw
  原文会把载荷撑大数倍，审计仍看 workflow_step.output 原值）。
- **前端**：左栏期次列表（日期 + 状态点金/红/灰 + 用时），"加载更早期次"
  按需拉第二页；主栏把 director-report-step 的正文提升为"本期周报·院长
  汇总"文档区，护理/总务/财务三步降级为"部门产出（支撑材料）"折叠块
  （默认收起，内含旧版全部渲染分支 + ⚙ 执行过程旁白）；选中态写 URL
  hash（#r=<id前8位>），刷新/分享可还原；<860px 侧栏变横向 chips。
- **验证中发现库里实有 11 期**（此前以为 1 期），含 cancelled 状态——
  补 runStat() 映射（cancelled=灰点"已取消"，不再掉进"进行中"闪烁），
  收尾时间 cancelled 显示"—"不显示"进行中"。
测试 120 passed 不变（API 改动无新 helper）；模板 bind-mount 即时生效，
容器因 main.py 改动重建。此页可作其他一拉到底页（告警/工单）的改造范式。

**同日十七补（四份产出统一为文档标签页）**：用户反馈主从改造后三个
部门产出"没有改成对应的样式"——它们被塞进默认收起的小折叠块，没得到
院长汇总的文档待遇（而总务/财务其实都是完整 markdown 报告）。改法：主栏
改标签页，四份产出同级——院长汇总（默认）/护理科·排班/总务科·物资配送/
财务科·成本预估，每个标签渲染同样的文档区（md 正文 + 状态徽章 + ⚙ 执行
...
## 工单页日期侧栏单行紧凑——长滑块二次收敛（滚动条归零）

用户二轮反馈：「滑动条还是太长了，还可以再短点」。app-shell 改造后三根
内层滚动条只剩两根，但日期侧栏滑块占比 62%（~385px）——16 个日期只溢出
375px 却占掉大半轨道，视觉上"一根粗棍子杵在那"。列表滑块 31% 属正常比例。

滑块长度 = 轨道 × clientHeight/scrollHeight，CSS 改不了占比——要"变短"
只有两条路：加内容量（不适用）或让内容全装下。日期项原是两行布局
（第一行 日期+标签，第二行 待N·总数），~46px/项；16 项 ≈ 736px > 栏高
601px。改成**单行紧凑**（`display:flex` 一行排 日期+标签+右侧"待N·N单"，
`white-space:nowrap` + ellipsis 防炸行），~31px/项，16 项 ≈ 500px 全装下。

复测（wo-verify2.cjs）：dates `canScroll:false`（sh 618 = ch 618，滚动条
彻底消失）；list 滑块 31% 不变；body 外层滚动仍归零；窄屏回退正常。
视觉终验（CDN 截图）：16 日期全可见无截断重叠，三栏等高无破版。smoke
全量 102 passed。

结论：全页现在只剩工单列表一根滚动条（暖色 8px，滑块 31% 标准比例），
日期栏 16 天一屏尽收。教训不变：先算占比再动手——两行紧凑方案算过
16×46px 仍溢出，直接否掉，省一轮返工。

## 工单页视觉自查——揪出单行日期统计截断（视觉模型漏报，DOM 实测命中）

用户要求截图自查「改得好看了吗」。视觉模型打 7.5 分并列五条扣分点，经
DOM 实测**四条为编造**：胶囊垂直居中偏差（实测 labelVsTagCenterDiff=0px，
flex align-items:center 下不可能不齐）、行高不一（16 行全部 32px 整）、
引用的文字格式/色值与实际 DOM 对不上。但它说「无明显截断」处，实测
**16 行统计全部 ellipsis 截断**——170px 侧栏内 label 47 + 标签 28 + 间隙
后统计只剩 49px，装不下 61px 的「待11 · 29 单」，每行丢 6-12px（"单"
字和末位数字被吃）。单行化的真实代价，上轮复测只量了滚动条没量文字宽。

修两处：侧栏 170px→190px（main 1fr 吸收 20px）；统计文字去空格
「待11 · 29 单」→「待11·29单」。复测 truncRows=0、dates canScroll
仍 false、全页仍只有 list 一根滚动条、外层滚动归零；smoke 102 passed。
...
## 2026-09-07 chat 意图关键词补人数类同义问句（销售自由问法编数修复）

- 聊天记录复盘发现的编数实例：销售问「院里住了多少人」，`_skill_queries`
  没有任何行命中 → 无 ERP 预取注入，agent 拿系统 prompt 里的对标口径
  （1300 余床）编出「约 1100 人」；真实库数据是 36 人。
- 修复（72758c3）：resident-query 行并入长词组 住了多少人/多少入住/
  入住人数/在院人数/在院老人数。**刻意不收裸「入住」**——首匹配 break
  会让 resident 行吞掉「入住率」类问句（床位/入住率口径另有大屏第⑤组件，
  不走预取）。新增回归钉模拟四种问法首匹配落 resident 行。122 测试绿。
...
### 同日补记：意图关键词二轮盘点（口语 battery，四组接入）

用户追问"类似的问题解决了吗"→ 22 条 battery 过匹配器：时间范围类
（最近三天菜单/告警/活动/工单、昨天值班、上月费用）上一轮已全覆盖；
盘出四组口语漏网并当日接入（ad34a37）：

| 组 | 补充关键词 | 要点 |
|---|---|---|
| 菜单 | 晚饭/早饭/午饭/夜宵/晚上吃什么…（整短语） | 裸"晚上"会吞"晚上有什么活动"→ 只收整短语 |
| 员工 | 护工/护士/医生/护理员 | "护理员"不含"护理等级"，评估行不吞 |
| 异常事件 | 摔倒/走失/坠床/噎食/发烧/发热 | **行必须在 resident（"老人"）之前**，否则"有老人摔倒吗"拿到的是老人名单 |
| 床位+财务 | 床位/入住率/空床/几床/空着→occupancy；收费/收了→summary | beds 端点一直现成没接，本次接上（上条残留②关闭） |

E2E 三连：「还有床位吗」→ 36/35/1/97% 与 occupancy 端点分毫不差；
「有老人摔倒吗」→ 命中沈桂花 09-03 摔倒危急未处理（真实 incident）；
「晚饭吃什么」→ 今晚 5 道菜+本周晚餐一览（真实菜单）。仍漏（刻意）：
「钱交了吗」（"交了吗"太泛）、「谁走失了」以外的低频事件词按需再补。

### 同日再补：回答风格收紧（问事实答事实，问分析才分析）

用户问"回答够准确且无废话吗"→ 实录统计：「晚饭吃什么」答 **1350 字/
7 张表/17 emoji**，一半以上是没问的内容（营养分析→4 条菜单优化建议→
3 份文档邀约→"请院长指示"）。与用户讨论后拍板收紧（d8d0cd3）：
**注入提示词加风格契约**（模块级 `_PROMPT_ANSWER_RULES`，员工非流式/
流式两处 + 家属 system prompt 同步）——首句直接答案、多行数据一张
简洁表格、不展开没问到的分析/建议、结尾不做邀约；仅当明确要建议才展开。

取舍理由记录在案：结尾问句在演示里是冷场点；数据系统的信任感来自克制
（"问什么答什么"本身是连着台账的信号）；流式下紧回答 ~3s 完事 vs
1350 字 10s+。真实数字/表格/严重度分级全保留——砍的只有三类废话。

收紧后实测：「晚饭」1350→451 字（仍多带一张本周排期表，轻微超纲但
可接受）；「床位」321 字首句即"2号楼 1 张空床"；「摔倒」214 字直接
命中 incident。数字与 ERP 仍分毫不差。要调回热闹风格 = 改一处常量。

### 同日四补：意图三轮自测 + 组合问句双行注入

用户要求自测补漏。扩展 battery（快速按钮文本 + 健康/用药 + 组合问句）
盘出 6 缺口，当日全部修复（131 测试 +9 全绿）：

| # | 缺口 | 修复 |
|---|------|------|
| 1 | 「某楼栋当班人员」（护理台快速按钮）半错命中员工行 | 排班行 +「当班」 |
| 2 | 「食堂今天做了什么」未命中 | 菜单行 +「食堂」 |
| 3 | resident 行人名"张建国"是陈旧种子名（库里没有）；演示主角张国栋反而不是关键词 | 换 nursing_residents 实有 8 人（张国栋/李秀兰/陈永发/赵玉芬/王淑珍/刘明德/吴桂英/周德胜） |
| 4 | 「有人投诉吗」未命中 | 新增投诉 SQL 行（nursing_complaints 本地 PG 3 行真实数据） |
| 5 | 「本日菜单和活动」（快速按钮）首匹配 break 只注一半 | `_match_skill_rows` 白名单组合对（meal+activity）双行注入，`_collect_skill_data` 统一两个端点入口 |
| 6 | 活动 SQL `date >= CURRENT_DATE` 在演示数据断档日空手 | 对齐 dashboard `_eff_date` 兜底（今天或未来有则取之，否则最近一天起） |

**行序重排（本轮最大坑）**：resident 行收进人名后，「吴桂英欠费」「食堂
有投诉」被人名/食堂吞掉——财务三行 + 投诉行整体提到 resident 行之前
（欠费→餐费→泛财务→投诉，意图更具体者前置）。撞车对全部有回归钉。

**刻意不做**：「院内通知」无数据源；「用药/血压」是 per-resident 端点、
列表预取喂不了（留给 agent 工具）；「钱交了吗」（"交了吗"太泛）。
组合白名单只放 meal+activity——任意双行并注会放大撞车（「评估盘点」
会连带注入库存），新组合确认无撞车再进白名单。

**E2E（院长/楼长/家属三会话全绿）**：「食堂今天做了什么」→ 真实三餐
表；「今天谁照顾张国栋」→ 档案表+诚实说明无照顾人字段；「有人投诉吗」
→ 3 条投诉两态统计表；「本日菜单和活动」→ 菜单+活动双数据；「吴桂英
欠费几个月」→ arrears 真数 3 个月；楼长 b1_liu 三连（床位 1号楼 6/6
满床、摔倒 0 但捞到张国栋血压预警、三天排班 6/5/5 每日汇总表）全部
X-Building 收窄；家属「这周吃饭情况」真实菜单周排。探针会话 16 条已清。

### 同日五补：演示准备两件套（自检脚本 + 问句手持卡）

- **`scripts/demo_day_check.sh`**：演示前 10 分钟一键自检——服务健康/
  ERP 匿名 401 红线/LLM 连通（欠费报文直指 switch_llm.sh）/数据新鲜度
  七项/三角色 7 问 chat 探针；`--quick` 跳探针 ~30s；红项退出码 1。
  首跑实测抓出 3 个真红项：告警停 09-03、工单停 09-03、活动停 08-25
  ——演示前需重锚（restore_demo.sh + seed_work_orders_demo.py）。
  坑两枚：week-menu 端点必须带 week_start（无参返回 []）；响应是纯
  list 不是 {items}，提取要兼容两形状。

### 同日六补：演示周全量备库（19 项全绿）

演示定在本周（09-07 当天执行）：①`restore_demo.sh` 全量重锚 ERP
（9014 单、告警锚定当天、演示位 0，备份 db.sqlite3.bak-20260907-165442）
②工单种子重跑（397 单/14 天含当天，当日完成率 16/30 留待办语义）
③**活动表铺 09-07~09-14 每日 4 场**（generate_series + 星期轮换讲座/
手工主题防复制感——此前只到 08-25，靠 _eff_date 兜底 chat 虽能答但
大屏卡面日期露馅）④院长会话触发周报（顶期次=当天）。备完 `demo_day_check.sh`
全量 19 绿 0 红 0 黄。**演示当天早上**：check 全绿 + 工单种子重跑 10s
（当日完成率语义）+ chat 随口一句预热（agent 网关首轮 init ~8s）。
- **`演示问句手册.md`**（仓库根，docs/ 被 gitignore）：三角色 15 分钟
  动线 + 稳问法分组（全部是踩过关键词验证的问法）+ 禁问清单（院内
  通知/用药/钱交了吗）+ 断网兜底。核心纪律：**销售必须照卡问**——
  自由问法踩不中关键词时 AI 诚实说查不到（不编数但冷场）。
  探针固定 chat_id=probe-$$，不产生侧栏会话。

### 同日七补：明天/后天排班假阴性（预取窗口无未来词 + 追问碎片零注入）

用户实测「明天后天呢」失败（AI 答"查询结果不含 09-08/09-09 排班数据"），
质问"数据是不是只写到今天"。核实：**ERP 排班已铺到 09-30**（09-08/09-09
各 22 行），数据没問題——锅在预取层两处：

1. **`_schedule_window` 只有过去词**（三天/最近/本周…），问到明天/后天
   返回 None → 预取只拉当天 → agent 拿着今日数据诚实报"没有明天的"。
   补未来词分支：明后天/明天后天→[+1,+2]、明天→[+1]、后天→[+2]、
   未来/接下来→[今天,+1,+2]。整词判断在单词之前（"明后天"同时含
   "明/后天"子串）。刻意**不把"明天"加进排班行关键词**——排班行在
   首位，会把「明天吃什么」吞成排班。
2. **裸追问碎片一行都不匹配**：「明天后天呢」不含排班/当班关键词，
   预取零注入（AI 是拿上一轮注入的旧数据在答）。`_collect_skill_data`
   加 history 参数 + 追问回退：本句无任何命中且含时间指代词
   （_FOLLOWUP_TIME_WORDS）→ 沿用上一句用户消息的意图、用**本句**的
   时间词重新取数。护栏：命中在即不回退（「明天吃什么」自己命中
   菜单行）。两个 chat 端点都传 `history=await _get_chat_msgs(chat_id)`。
3. **顺带补菜单行泛短语「吃什么」**：测试暴露「明天吃什么」原本也是
   零命中（菜单行只收整短语）——既是菜单预取缺口，也会让回退护栏
   失效（排班追问后问"明天吃什么"会被带回排班）。「吃什么」一个
   子串覆盖 今天/明天/晚上吃什么 全系问法。

测试 +2（未来窗口展开 + 追问回退含不回退护栏），133 全绿，ruff 基线
持平；dato-control 重建（首跑 pip install Pillow 网络抖动失败，重跑过）。
E2E（院长，流式）：「最近三天排班情况」→ 三日汇总表；同会话追问
「明天后天呢」→ 09-08/09-09 各 22 人含楼栋明细（与 ERP 实数分毫不差）；
独立问「明天谁当班」→ 明日 22 人分班表。探针会话已清。

### 同日八补：流式输出卡顿感（前端平滑排空）

用户反馈"流式输出有点卡顿"。先实测定位（公网 chat.eldcare.cn:8443
curl 计时每 delta 到达间隔）：**网络链路不攒块**（中位 97ms）、服务端
逐 delta 透传——卡顿源是**上游出块节奏不均**（p90 314ms、偶发 2.8s
停顿，一块平均 20 字），而前端"到多少显多少"，文字就一阵一阵跳。

修法（纯前端 chat.html，模板 bind-mount 即时生效）：**平滑排空缓冲**——
delta 进 `acc`，30ms 定时器按 `step = max(1, ceil(remain/6))` 匀速吐字
（基础 ~133 字/s，积压越多步长越大、≤0.2s 内追平，既顺滑又不拖后）；
done/error/流中断三处停表并整段终渲染。Playwright 采样验证：增长步长
中位数 7 字/50ms（= 匀速打字节奏）、无 >60 字爆点（改前是 20 字/块、
100-300ms 一跳）。排障脚本选型：登录页字段 #login-user/#login-pass、
...
- 新增 `docs/运维/内网项目公网接入手册.md`（原名「公网体验环境复刻手册」，09-22 改）：
  frp 内网穿透公网接入全流程
