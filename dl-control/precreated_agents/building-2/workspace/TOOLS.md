...
Skills define *how* tools work. This file describes the tools available to 2号楼楼长.
...
1. **晨间巡查**: `nursing-work-order` 查昨日工单完成情况 → `alert-query` 查新增预警 → 实地巡查各房间（重点关注失智老人）
2. **失智老人管理**: `resident-query` 查失智老人档案 → `nursing-work-order` 确认防走失巡查工单 → 检查定位手环状态
