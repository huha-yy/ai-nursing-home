...
Skills define *how* tools work. This file describes the tools available to 6号楼楼长.
...
1. **晨间巡查**: `nursing-work-order` 查昨日工单完成情况 → `alert-query` 查新增预警 → 实地巡查各房间（重点关注心电监护老人）
2. **重症监护**: `resident-query` 查R031档案 → `nursing-work-order` 核实心电监护/出入量工单 → 关注心电图异常预警
3. **氧疗管理**: `logistics-query` 查吸氧管库存 → 核实R034、R025用氧工单 → 确保持续供氧
