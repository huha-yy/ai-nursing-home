...
export function JobsWatchPage() {
  ...
  if (!snap) {
    return <div style={{ padding: 24, color: 'var(--text-muted, #777)' }}>{t('jobs.loading')}</div>;
  }
    ...
    <div style={{ padding: 24, fontFamily: 'var(--font-mono, "JetBrains Mono", monospace)' }}>
      <h1 style={{ fontSize: 18, marginBottom: 4 }}>
        ...
        <span style={{ marginLeft: 12, color: 'var(--text-muted, #777)', fontSize: 12, fontWeight: 'normal' }}>
          {t('jobs.updated', { time: ts })}
        </span>
      </h1>

      <section style={{ marginTop: 24 }}>
        <h2 style={{ fontSize: 14, marginBottom: 8 }}>{t('jobs.section.queue')}</h2>
        <div>
          {t('jobs.queue.waiting', { n: snap.queue_health.waiting })}{'  '}
          {t('jobs.queue.active', { n: snap.queue_health.active })}{'  '}
          <span style={{ color: snap.queue_health.stalled > 0 ? 'var(--accent-warn, #d29922)' : undefined }}>
            {t('jobs.queue.stalled', { n: snap.queue_health.stalled })}
          </span>
        </div>
      </section>
      ...
      {snap.by_type.length > 0 && (
        <section style={{ marginTop: 24 }}>
          <h2 style={{ fontSize: 14, marginBottom: 8 }}>{t('jobs.section.byType')}</h2>
          <table style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ color: 'var(--text-muted, #777)', fontSize: 12 }}>
                <th style={{ textAlign: 'left', padding: '4px 12px 4px 0' }}>{t('jobs.table.name')}</th>
                <th style={{ textAlign: 'right', padding: '4px 12px' }}>{t('jobs.table.total')}</th>
                <th style={{ textAlign: 'right', padding: '4px 12px' }}>{t('jobs.table.done')}</th>
                <th style={{ textAlign: 'right', padding: '4px 12px' }}>{t('jobs.table.fail')}</th>
                <th style={{ textAlign: 'right', padding: '4px 12px' }}>{t('jobs.table.dead')}</th>
              </tr>
            </thead>
            <tbody>
              {snap.by_type.slice(0, 6).map(tRow => (
                <tr key={tRow.name}>
                  <td style={{ padding: '4px 12px 4px 0' }}>{tRow.name}</td>
                  <td style={{ textAlign: 'right', padding: '4px 12px' }}>{tRow.total}</td>
                  <td style={{ textAlign: 'right', padding: '4px 12px' }}>{tRow.completed}</td>
                  <td style={{ textAlign: 'right', padding: '4px 12px' }}>{tRow.failed}</td>
                  <td style={{ textAlign: 'right', padding: '4px 12px' }}>{tRow.dead}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section style={{ marginTop: 24 }}>
        <h2 style={{ fontSize: 14, marginBottom: 8 }}>{t('jobs.section.leasePressure')}</h2>
        <div style={{ color: leasePressureColor(snap.lease_pressure_1h) }}>
          {t('jobs.leasePressure.bounces', { n: snap.lease_pressure_1h, suffix: snap.lease_pressure_1h === 1 ? '' : 'es' })}
        </div>
      </section>

      {snap.top_errors.length > 0 && (
        <section style={{ marginTop: 24 }}>
          <h2 style={{ fontSize: 14, marginBottom: 8 }}>{t('jobs.section.topErrors')}</h2>
          <table style={{ borderCollapse: 'collapse' }}>
            <tbody>
      ...
      {snap.budget_owners.length > 0 && (
        <section style={{ marginTop: 24 }}>
          <h2 style={{ fontSize: 14, marginBottom: 8 }}>{t('jobs.section.budgetOwners')}</h2>
          <table style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ color: 'var(--text-muted, #777)', fontSize: 12 }}>
                <th style={{ textAlign: 'left', padding: '4px 12px 4px 0' }}>{t('jobs.budgetTable.owner')}</th>
                <th style={{ textAlign: 'right', padding: '4px 12px' }}>{t('jobs.budgetTable.spent')}</th>
                <th style={{ textAlign: 'right', padding: '4px 12px' }}>{t('jobs.budgetTable.remaining')}</th>
              </tr>
            </thead>
            <tbody>
