...
export function buildFeishuAgentBody(params: {
  ctx: Pick<
    FeishuMessageContext,
    "content" | "senderName" | "senderOpenId" | "chatId" | "mentionTargets" | "messageId" | "hasAnyMention"
  >;
  ...
  // Inject Feishu conversation metadata so the agent knows which chat it's in.
  // This enables tools like send_image.py to send images back to the right chat.
  messageBody += `\n[FeishuChatId: ${ctx.chatId}]`;
  messageBody += `\n[FeishuSenderOpenId: ${ctx.senderOpenId}]`;
...
export async function handleFeishuMessage(params: {
  ...
  // Handle audio messages: download via Feishu API, transcribe via Whisper, inject text
  if (event.message.message_type === "audio") {
    try {
      const audioContent = JSON.parse(event.message.content);
      const fileKey = audioContent.file_key;
      if (fileKey) {
        log(`feishu[${account.accountId}]: processing audio message, file_key=${fileKey}`);
        const pythonScript = "/opt/openclaw/scripts/transcribe_audio.py";
        const result = execSync(
          `python3 "${pythonScript}" --message-id "${event.message.message_id}" --file-key "${fileKey}"`,
          { timeout: 300000, maxBuffer: 10 * 1024 * 1024, encoding: "utf-8" },
        );
        const transcribed = result.trim();
        if (transcribed && !transcribed.startsWith("[语音转写失败")) {
          ctx = { ...ctx, content: `[语音消息转写] ${transcribed}` };
          log(`feishu[${account.accountId}]: audio transcribed (${transcribed.length} chars)`);
        } else {
          log(`feishu[${account.accountId}]: audio transcription failed: ${transcribed}`);
        }
      } else {
        log(`feishu[${account.accountId}]: audio message missing file_key in content`);
      }
    } catch (err) {
      log(`feishu[${account.accountId}]: audio processing failed: ${String(err)}`);
    }
  }

  // Handle image messages: download and save to /tmp/, then run OCR
  if (event.message.message_type === "image") {
    try {
      const imgContent = JSON.parse(event.message.content);
      const imgKey = imgContent.image_key;
      if (imgKey) {
        log(`feishu[${account.accountId}]: processing image message, image_key=${imgKey}`);
        // Download image using Feishu API via Python helper
        const pythonScript = "/opt/openclaw/scripts/download_image.py";
        const result = execSync(
          `python3 "${pythonScript}" --message-id "${event.message.message_id}" --image-key "${imgKey}"`,
          { timeout: 60000, maxBuffer: 10 * 1024 * 1024, encoding: "utf-8" },
        );
        const imagePath = result.trim();
        if (imagePath && !imagePath.startsWith("[错误")) {
          log(`feishu[${account.accountId}]: image saved to ${imagePath}`);
          // Append image path hint to content so the agent can reference it
          ctx = { ...ctx, content: `${ctx.content}\n\n[图片路径: ${imagePath}]` };

          // Try OCR in background (best-effort, don't block dispatch)
          try {
            const ocrResult = execSync(
              `python3 -c "import sys; sys.path.insert(0,'/opt/openclaw/skills/custom/vision-ocr'); from handler import ocr; print(ocr('${imagePath}'))"`,
              { timeout: 60000, maxBuffer: 10 * 1024 * 1024, encoding: "utf-8" },
            );
            const ocrText = ocrResult.trim();
            if (ocrText && !ocrText.startsWith("[") && !ocrText.startsWith("[错误") && !ocrText.startsWith("[OCR")) {
              ctx = { ...ctx, content: `${ctx.content}\n\n[图片OCR: ${ocrText}]` };
              log(`feishu[${account.accountId}]: image OCR (${ocrText.length} chars)`);
            }
          } catch (ocrErr) {
            log(`feishu[${account.accountId}]: image OCR failed: ${String(ocrErr)}`);
          }
        } else {
          log(`feishu[${account.accountId}]: image download failed: ${imagePath}`);
        }
      } else {
        log(`feishu[${account.accountId}]: image message missing image_key in content`);
      }
    } catch (err) {
      log(`feishu[${account.accountId}]: image processing failed: ${String(err)}`);
    }
  }

  // Handle file messages: download, parse, classify, and store to GBrain
  if (event.message.message_type === "file") {
    try {
      const fileContent = JSON.parse(event.message.content);
      const fileKey = fileContent.file_key;
      if (fileKey) {
        log(`feishu[${account.accountId}]: processing file message, file_key=${fileKey}`);
        const pythonScript = "/opt/openclaw/scripts/parse_and_store.py";
        const result = execSync(
          `python3 "${pythonScript}" --message-id "${event.message.message_id}" --file-key "${fileKey}"`,
        ...
        if (output && !output.startsWith("[错误")) {
          log(`feishu[${account.accountId}]: file parsed and stored — ${output}`);
          ctx = { ...ctx, content: `${ctx.content}\n\n[文件已入库: ${output}]` };
        } else {
          log(`feishu[${account.accountId}]: file processing failed: ${output}`);
        }
      } else {
        log(`feishu[${account.accountId}]: file message missing file_key in content`);
      }
    } catch (err) {
      log(`feishu[${account.accountId}]: file processing failed: ${String(err)}`);
    }
  }
  ...
  if (isGroup) {
    ...
    // Multi-bot group guard: if the message @-mentions someone but not *this* bot,
    // skip it.  Without this check every bot in a shared group replies to @-messages
    // intended for other bots when groupPolicy=open.
    if (isGroup && ctx.hasAnyMention && !ctx.mentionedBot) {
      log(
        `feishu[${account.accountId}]: message in group ${ctx.chatId} mentions other entity but not this bot; skipping`,
      );
  ...
  try {
      ...
      if (dmPolicy === "pairing") {
        await pairing.issueChallenge({
          ...
          sendPairingReply: async (text) => {
            // Extract pairing code from SDK-generated text (e.g.
            // "Pairing request sent! Your verification code is: ABCD")
            // then send a Chinese-localized message instead.
            const codeMatch = text.match(/code\s*is\s*:?\s*(\w+)/i);
            const code = codeMatch?.[1] ?? "unknown";
            const chineseText =
              `🔐 配对验证\n\n` +
              `您需要先完成配对才能与机器人对话。\n\n` +
              `**您的配对验证码是：\`${code}\`**\n\n` +
              `请将此验证码发送给管理员，管理员在后台审批通过后您即可正常使用。\n\n` +
              `⏱ 验证码有效期为 1 小时。`;
    ...
    if (broadcastAgents) {
      ...
      // Notify user when LLM returned 0 replies (e.g., empty response, 0 tokens)
      if (counts.final === 0 && !queuedFinal) {
        try {
          await sendMessageFeishu({
            cfg,
            to: ctx.chatId,
            text: "⚠️ AI 未能生成回复，可能是服务异常或内容被过滤。请稍后重试。",
            accountId: account.accountId,
          });
        } catch (noticeErr) {
          error(`feishu[${account.accountId}]: failed to send zero-reply notice: ${String(noticeErr)}`);
        }
      }
    }
  } catch (err) {
