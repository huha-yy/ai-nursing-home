  ...
  const { dispatcher, replyOptions, markDispatchIdle } =
    core.channel.reply.createReplyDispatcherWithTyping({
      ...
      onError: async (error, info) => {
        ...
        // Send visible error notice to user instead of silent failure
        const errMsg = error instanceof Error ? error.message : String(error);
        let notice: string;
        if (errMsg.includes("1033") || errMsg.includes("system error")) {
          notice = `⚠️ AI 服务暂时不可用（系统错误），请稍后重试。`;
        } else if (errMsg.includes("timeout") || errMsg.includes("ETIMEDOUT")) {
          notice = `⚠️ AI 服务响应超时，请稍后重试。`;
        } else if (errMsg.includes("rate limit") || errMsg.includes("429")) {
          notice = `⚠️ AI 服务请求频繁，请稍等片刻再试。`;
        } else {
          notice = `⚠️ 消息处理失败：${errMsg.length > 200 ? errMsg.slice(0, 200) + "…" : errMsg}`;
        }
        try {
          await sendMessageFeishu({ cfg, to: chatId, text: notice, accountId });
        } catch (_) {
          // error notice delivery failed — already logged above
        }
      },
      onIdle: async () => {
        await closeStreaming();
