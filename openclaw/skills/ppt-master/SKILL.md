...
===
🚀 快速模式（DEFAULT — 简单场景/已知品牌）
===

用户要求已够明确（知道品牌、有明确主题、页数 <15），走此模式。
**原则：不提问、不确认、一步到底。**

流程：
1. 读取知识库中该品牌资料（品牌名、定位、产品线）
2. 自己确定配色/风格（品牌色优先，没有则 glassmorphism 冰蓝深空）
3. 直接生成 Design Spec → spec_lock → SVG 10页 → 导出 PPTX
4. 如果有图片需求，调用 comfy_mcp_server generate_image(hd=False) 快速出图
5. 用 send_file.py --to "[FeishuChatId]" --type chat_id 发送

**通信规则：只报告关键节点，不列选项、不提问、不解释。**
关键节点：开始 → 生成完毕，中间静默。例如：
"✅ 戴恩科技介绍 PPT 已生成，10页，正在发送..."
"📎 文件已发送"
...
用户已经有一个 ppt-master 生成的项目，要求修改时走此模式。
**原则：用户说什么改什么，不改的不碰。**
...
| 用户说 | 操作 |
|:-------|:-----|
| "第三页换个颜色" → `update_spec.py` 改色+重导出 |
| "改成红色主题" → `update_spec.py <project> primary=#CC0000` |
| "字体改大" → 编辑 spec_lock.md 中 body 字号 → `update_spec.py` 同步 |
| "第二页改文字内容" → 编辑 `svg_output/02_*.svg` 中的文字 → 重跑导出 |
| "加一页关于XX" → 在 svg_output/ 新增 `11_XX.svg`（参考已有页的样式）→ spec_lock.md 加 page_rhythm → 重跑导出 |
| "删掉第五页" → 删 `svg_output/05_*.svg` → 导出 |
| "换封面图" → ComfyUI generate_image(hd=False) → 嵌入 `01_封面.svg` → 导出 |
| "发给我" → send_file.py 重发即可 |

**重导出命令：**
```bash
cd <project_path>
python3 /opt/openclaw/skills/custom/ppt-master/scripts/svg_to_pptx.py . -o exports/updated.pptx
```

**通信规则：修改完成了说"已改好"，需要确认才问一句。**
"已改好，重新发了 📎"
"封面图要什么样的？一句话描述就行"
